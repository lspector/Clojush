...
(def individuals-per-error (atom nil))
; used for lexicase parent selection to randomly select one individual
; for each error cases
; added as a global so we only calculate once per generation
