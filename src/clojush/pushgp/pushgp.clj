...
(def push-argmap
     (atom (sorted-map :error-function (fn [p] '(0)) ;; pgm -> list of errors (1 per case)
                       ...
                       :use-condensed-lexicase-selection false
                       :use-rmse false
     ...
     (binding [*thread-local-random-generator* (java.util.Random. (:random-seed @push-argmap))]
       ...
       (let [{:keys [pop-agents child-agents rand-gens]} (make-agents-and-rng @push-argmap)]
         ...
         (loop [generation 0]
           ...
           ;; create global structure to support condensed lexicase selection
           (when @global-use-condensed-lexicase-selection 
             (extract-lexicase-reachable-subsets pop-agents))
