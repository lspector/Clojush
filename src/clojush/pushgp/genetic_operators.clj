...
(defn uniform-mutation
  ...
  (let [uniform-mutation-rate 
        (random-element-or-thing uniform-mutation-rate)
        
        uniform-mutation-constant-tweak-rate 
        (random-element-or-thing uniform-mutation-constant-tweak-rate)
        
        uniform-mutation-float-gaussian-standard-deviation 
        (random-element-or-thing uniform-mutation-float-gaussian-standard-deviation)
        
        uniform-mutation-int-gaussian-standard-deviation 
        (random-element-or-thing uniform-mutation-int-gaussian-standard-deviation)
        
        uniform-mutation-tag-gaussian-standard-deviation 
        (random-element-or-thing uniform-mutation-tag-gaussian-standard-deviation)
        
        uniform-mutation-string-char-change-rate 
        (random-element-or-thing uniform-mutation-string-char-change-rate)
...
(defn uniform-instruction-mutation
  ...
  [ind {:keys [uniform-mutation-rate maintain-ancestors atom-generators]
        :as argmap}]
  (let [uniform-mutation-rate (random-element-or-thing uniform-mutation-rate)
        instruction-mutator (fn [token]
                              (assoc token
                                :instruction
                                (:instruction 
...
(defn uniform-boolean-mutation
  ...
  [ind {:keys [uniform-mutation-constant-tweak-rate 
               maintain-ancestors atom-generators]
        :as argmap}]
  (let [uniform-mutation-constant-tweak-rate (random-element-or-thing uniform-mutation-constant-tweak-rate)
        constant-mutator (fn [token]
                           (let [const (:instruction token)]
...
(defn uniform-close-mutation
  ...
  (if (not (some #{:close} epigenetic-markers))
    ind
    (let [uniform-close-mutation-rate (random-element-or-thing uniform-close-mutation-rate)
          close-increment-rate (random-element-or-thing close-increment-rate)
...
(defn uniform-silence-mutation
  ...
  (if (not (some #{:silent} epigenetic-markers))
    ind
    (let [uniform-silence-mutation-rate (random-element-or-thing uniform-silence-mutation-rate)
...
given by uniform-deletion-rate."
  ...
  (let [rate (random-element-or-thing uniform-deletion-rate)
...
(defn uniform-addition
  ...
  (let [rate (random-element-or-thing uniform-addition-rate)
...
(defn uniform-addition-and-deletion
  ...
  [ind {:keys [uniform-addition-and-deletion-rate maintain-ancestors atom-generators] 
        :as argmap}]
  (let [addition-rate (random-element-or-thing uniform-addition-and-deletion-rate)
...
(defn uniform-combination
  ...
  [parent1 parent2
   {:keys [uniform-combination-rate maintain-ancestors]
    :as argmap}]
  (let [combination-rate (random-element-or-thing uniform-combination-rate)
        new-genome (vec
                    (apply concat
                           (mapv (fn [g1 g2]
...
(defn uniform-combination-and-deletion
  ...
  [parent1 parent2 
   {:keys [uniform-combination-and-deletion-rate maintain-ancestors]
    :as argmap}]
  (let [combination-rate (random-element-or-thing uniform-combination-and-deletion-rate)
...
(defn alternation
  ...
  (let [alternation-rate (random-element-or-thing alternation-rate)
        alignment-deviation (random-element-or-thing alignment-deviation)
        s1 (:genome parent1)
        s2 (:genome parent2)
        new-genome (loop [i 0
