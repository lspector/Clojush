...
;        c1c (produce-child-genome-by-autoconstruction c1 g true argmap)
;        c2c (produce-child-genome-by-autoconstruction c2 g true argmap)]
...
(defn compute-errors
  [g {:keys [error-function pass-individual-to-error-function] :as argmap}]
  (let [p (translate-plush-genome-to-push-program {:genome g} argmap)]
    (if pass-individual-to-error-function
      (error-function (make-individual :genome g :program p))
      (error-function p))))
...
;         (apply distinct? [(expressed-difference c1 c1c argmap)
;                           (expressed-difference c2 c2c argmap)])
;         (distinct? (compute-errors g argmap)
;                    (compute-errors c1c argmap)
;                    (compute-errors c2c argmap)))))
...
;        c1c (produce-child-genome-by-autoconstruction c1 g true argmap)
;        c2c (produce-child-genome-by-autoconstruction c2 g true argmap)]
;    (and (distinct? (expressed-difference c1 c1c argmap)
;                    (expressed-difference c2 c2c argmap))
;         (distinct? (compute-errors g argmap)
;                    (compute-errors c1c argmap)
;                    (compute-errors c2c argmap)))))
...
;  "Returns true iff genome g is considered recursively variant."
;  [g argmap]
;  (let [c (produce-child-genome-by-autoconstruction g g false argmap)
;        gc (produce-child-genome-by-autoconstruction c c false argmap)]
;    (and (distinct? (expressed-difference g c argmap)
;                    (expressed-difference c gc argmap))
;         (distinct? (compute-errors g argmap)
;                    ;(compute-errors c argmap)
;                    (compute-errors gc argmap)))))

;(defn recursively-variant?
;  "Returns true iff genome g is considered recursively variant."
;  [g argmap]
;  (let [translate #(translate-plush-genome-to-push-program {:genome %} argmap)
;        c (produce-child-genome-by-autoconstruction g g false argmap)
;        c2 (produce-child-genome-by-autoconstruction g g false argmap)
;        gc (produce-child-genome-by-autoconstruction c c false argmap)]
;    (and (distinct? (expressed-difference g c argmap)
;                    (expressed-difference c gc argmap))
;         (apply distinct? (map translate [g c c2 gc]))
;         (let [g-errors (compute-errors g argmap)
;               c-errors (compute-errors c argmap)
;               gc-errors (compute-errors gc argmap)]
;           (distinct? (map - g-errors c-errors)
;                      (map - c-errors gc-errors))))))
...
;  [g argmap]
;  (let [num-kids 5
;        kids (repeatedly num-kids 
;                         #(produce-child-genome-by-autoconstruction g g false argmap))
;        diffs (map #(expressed-difference g % argmap) kids)]
;    (and (> (reduce min diffs) 0)
;         (> (count (distinct diffs)) 1)
