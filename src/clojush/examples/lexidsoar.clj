...
(defn crossover 
  ...
  (let [new-program (case (lrand-int 3)
                      0 (insert-code-at-point 
                          (:program parent1) 
                          (select-node-index (:program parent1))
                          (code-at-point (:program parent2)
                                         (select-node-index (:program parent2))))
                      1 (list 'if-dirty (:program parent1) (:program parent2))
                      2 (list 'if-obstacle (:program parent1) (:program parent2)))]
    (if (> (count-points new-program) max-points)
      parent1
...
(in-ns 'clojush.examples.lexidsoar)
...
(pushgp
  ...
  :reproduction-simplifications 50
  :max-points 500
