...
(defn silence-germline
  "Returns genome g with the :silent marker set to true for all germline instructions."
  [g]
  (mapv (fn [instruction-map]
          (if (some #{:genome}
                    (:stack-types (meta (get @instruction-table 
                                             (:instruction instruction-map)))))
