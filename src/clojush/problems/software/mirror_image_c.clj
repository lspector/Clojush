...
(ns clojush.problems.software.mirror-image-c
      ...
      (doall
       (for [[[input1 input2] output] cases]
         (let [final-state (run-push program
                                     (->> (assoc (make-push-state) :calculate-mod-metrics (= [[input1 input2] output] ran))
                                          (push-item input2 :input)
                                          (push-item input1 :input)))
               _ (if (= [[input1 input2] output] ran)
                   (let [;_ (prn (:trace final-state))
                         ;_ (prn (:trace_id final-state))
           ...
           (top-item :boolean final-state)))))  (doall @reuse-metric) (doall @repetition-metric)   )))
  ...
  ([individual data-cases] ;; data-cases should be :train or :test
   (let [cases (case data-cases
         ...
         behaviors (mirror-image-evaluate-program-for-behaviors (:program individual)
                                                                 cases)
         errors (mirror-image-errors-from-behaviors (first behaviors) cases)]
     (cond
       (= data-cases :train) (assoc individual :behaviors behaviors :errors errors :reuse-info (nth behaviors 1) :repetition-info (last behaviors))
