(ns clojush.mod-metrics6
  (:use [clojush.util]
    ...
    ;[clojure.math.numeric-tower]
        ...
        [clojush.problems.software.last-index-of-zero-c :as liz]
...
(defn main-func
  [num]
  (dotimes [n num]
    (let [args (assoc liz/argmap :max-genome-size-in-initial-program 50 :genome-representation :plush)
          ind (translate-plush-genome-to-push-program (genesis args) args)
          error-func (get liz/argmap :error-function)
          modified-ind (error-func {:program ind}  :train)
          ]
      (prn (reduce + (:reuse-info modified-ind)) (count (filter #(= % 0) (:errors modified-ind)) )  ))))