...
(defn limit-stack-depths
  [push-state]
  (into (make-push-state) 
        (for [[k v] push-state] 
          [k (if (sequential? v)
               (apply list (take 10 v))
               v)])))
...
(defn eval-push 
  ...
  ([state print-steps trace save-state-sequence]
   ...
   ;  (println "global-atom-generators is empty. You should do something like: 
   ;(reset! global-atom-generators '(exec_if boolean_not true false))"))
   (loop [iteration 1
          s (limit-stack-depths state)
