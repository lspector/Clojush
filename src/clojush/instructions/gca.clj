...
GCA_maybe (takes/stores float 0-1, matches if rand <)
GCA_random (if stack all true replaces w/random gene, either way clears stack)
GCA_mate (takes/stores integer, if stack all true replace w/mate's gene @ offset, either way clear stack)
GCA_self (takes/stores integer, if stack all true replace w/own gene @ offset, either way clear stack)
GCA_silent (takes/stores boolean, if stack all true sets silent to boolean)
GCA_erase (if stack all true make gene nil, which will be deleted if not further changed; clear stack)
GCA_dup_left (if stack all true make dup on left, max once (last) per event; clear stack)
GCA_dup_right (if stack all true make dup on right, max once (last) per event; clear stack)
